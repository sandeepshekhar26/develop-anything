...
// AST Parser — regex-based extraction for zero-dep portability
...
export function parseFile(entry: FileEntry): ParsedFile {
  let content: string;
  try {
...
function emptyParsedFile(entry: FileEntry): ParsedFile {
  return { entry, imports: [], exports: [], symbols: [], comments: [] };
}

/** Parse all files in batch */
export function parseFiles(entries: FileEntry[]): ParsedFile[] {
  ...
  logger.debug(`Parsed ${results.length} files`);
  return results;
}
